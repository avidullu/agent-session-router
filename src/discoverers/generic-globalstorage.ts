...
function getRoots(id: string): string[] {
    ...
    r.push(path.join(os.homedir(), 'Library', 'Application Support', 'Code', 'User', 'globalStorage', id));
    return r;
}

function discoverFromRoots(id: string, kind: string, sourceName: string): () => AsyncIterable<DiscoveredSession> {
            ...
            const collect = (dir: string, d: number): void => {
                ...
                try {
                    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
                        ...
                        if (e.isDirectory()) { collect(p, d + 1); continue; }
                        if (!e.name.endsWith('.json') && !e.name.endsWith('.jsonl')) continue;
                        try {
                            const s = fs.statSync(p);
                            results.push({ sourceName, sourceKind: kind, filePath: p, sessionId: e.name.replace(/\.(json|jsonl)$/, ''), sizeBytes: s.size, mtimeMs: s.mtimeMs });
                        } catch { /* skip */ }
                    }
                } catch { /* skip */ }
            };
...
registerDiscoverer('amazon_q', discoverFromRoots('amazonwebservices.amazon-q-vscode', 'amazon_q', 'amazon-q'));
